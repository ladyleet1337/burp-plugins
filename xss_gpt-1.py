...
class BurpExtender(IBurpExtender, IScannerCheck):
    ...
    def create_issue(self, baseRequestResponse, payload):
        ...
        payload_start = self._helpers.indexOf(baseRequestResponse.getRequest(), payload, True, 0, len(baseRequestResponse.getRequest()))
        payload_end = payload_start + len(payload)

        return CustomScanIssue(
            http_service,
            url,
            [self._callbacks.applyMarkers(baseRequestResponse, None, [payload_start, payload_end])],
            issue_name,
            "A potential SSTI/XSS vulnerability was detected. Payload: {}".format(payload),
            "High",
            "") # <--- add the missing closing quotation mark here
